...
public class PostServiceTests
{
    ...
    public async Task CadastrarPost_DeveCriarPost()
    {
        ...
        _mapperMock.Setup(m => m.Map<List<Comentario>>(dto.Comentarios)).Returns(comentarios);
        _mapperMock.Setup(m => m.Map<PostRequestDTO>(It.IsAny<Post>())).Returns(dto);
    ...
    public async Task ListarPosts_DeveRetornarListaMapeada()
    {
        ...
        {
            Post.Criar("Titulo", new List<Comentario>(), 1, 0)
        };

        var dto = new List<PostResponseDTO> { new PostResponseDTO() };

        _authServiceMock.Setup(a => a.ObterUsuarioAutenticado()).ReturnsAsync(usuario);
        _repositoryMock.Setup(r => r.ListarPosts(usuario.Id)).ReturnsAsync(posts);
        _mapperMock.Setup(m => m.Map<List<PostResponseDTO>>(posts)).Returns(dto);
    ...
    {
        var usuario = new Usuario("Pedro", "email@email.com", "123");
        var post    = Post.Criar("Titulo", new List<Comentario>(), usuario.Id, 0);

        var dto = new ComentarioRequestDTO { PostId = post.Id, Descricao = "Comentário" };

        _authServiceMock.Setup(a => a.ObterUsuarioAutenticado()).ReturnsAsync(usuario);
        _repositoryMock.Setup(r => r.ObterPostPorId(dto.PostId)).ReturnsAsync(post);
        _mapperMock.Setup(m => m.Map<ComentarioRequestDTO>(It.IsAny<Comentario>())).Returns(dto);
    ...
    public async Task ListarPostApenasDosAmigos_DeveRetornarPostsDosAmigos()
    {
        ...
        var posts = new List<Post> { Post.Criar("Titulo", new List<Comentario>(), 2, 0) };
        var dto   = new List<PostResponseDTO> { new PostResponseDTO() };

        _authServiceMock.Setup(a => a.ObterUsuarioAutenticado()).ReturnsAsync(usuario);
        _amizadeRepositoryMock.Setup(a => a.ObterIdsDosAmigos(usuario.Id)).ReturnsAsync(idsAmigos);
        _repositoryMock.Setup(r => r.ListarPostsDosAmigos(idsAmigos)).ReturnsAsync(posts);
        _mapperMock.Setup(m => m.Map<List<PostResponseDTO>>(posts)).Returns(dto);
