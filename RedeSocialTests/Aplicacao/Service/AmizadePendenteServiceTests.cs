...
public class AmizadePendenteServiceTests
{
    ...
    public async Task ListarPedidosRecebidos_DeveRetornarListaMapeada()
    {
        ...
        var pedidos = new List<AmizadePendente>
        {
            new AmizadePendente().Criar(2, usuario.Id)
        };

        var dto = new List<AmizadePendenteDTO>
        {
            new AmizadePendenteDTO()
        };

        _authServiceMock.Setup(a => a.ObterUsuarioAutenticado())
            .ReturnsAsync(usuario);

        _repositoryMock.Setup(r => r.ListarPedidosRecebidos(usuario.Id))
            .ReturnsAsync(pedidos);

        _mapperMock.Setup(m => m.Map<List<AmizadePendenteDTO>>(pedidos))
            .Returns(dto);
    ...
    public async Task ListarPedidosEnviados_DeveRetornarListaMapeada()
    {
        ...
        var pedidos = new List<AmizadePendente>
        {
            new AmizadePendente().Criar(usuario.Id, 2)
        };
        ...
        _repositoryMock.Setup(r => r.ListarPedidosEnviados(usuario.Id))
            .ReturnsAsync(pedidos);
    ...
    public async Task EnviarPedidoDeAmizade_DeveCriarPedido_QuandoValido()
    {
        ...
        var dto = new AmizadePendenteDTO
        {
            DestinatarioId = 2
        };
        ...
        _repositoryMock.Setup(r => r.ValidarSeJaTemPedido(usuario.Id, dto.DestinatarioId))
            .ReturnsAsync(false);

        _amizadeRepositoryMock.Setup(a => a.JaSaoAmigos(usuario.Id, dto.DestinatarioId))
            .ReturnsAsync(false);

        _repositoryMock.Setup(r => r.EnviarSolicitacao(It.IsAny<AmizadePendente>()))
            .ReturnsAsync(entidade);

        _mapperMock.Setup(m => m.Map<AmizadePendenteDTO>(entidade))
            .Returns(dto);
    ...
    public async Task RejeitarPedido_DeveChamarRepositorio()
    {
        ...
        _repositoryMock.Setup(r => r.ObterPedidoDeAmizadePorId(1))
            .ReturnsAsync(pedido);
